...
6b. **Keyboard hotplug**: the evdev listener only enumerates devices when it starts. `_restart_listener_on_hotplug` watches the `/dev/input` directory mtime each idle tick and restarts the listener when the matching device set changes (wireless dongles and Bluetooth keyboards often (re)appear after suspend/resume - this was a real field failure). Do not remove it. Two guards protect it from the udev race (a new node exists for tens of ms before udev chowns it to the `input` group - a real field failure with a USB keyboard, 2026-09-02): the check waits until the directory mtime is `HOTPLUG_SETTLE_SECONDS` old, and while `scan_hotkey_devices` reports unreadable nodes it does not record the mtime, so the next tick rescans, for at most `HOTPLUG_RETRY_SECONDS`. Both constants live in `service.py`; tests in `tests/test_service_hotplug.py`.
